...
| Beautiful | Dark-first war-room aesthetic, monospace order log, one saturated accent per power, motion that respects `prefers-reduced-motion`. |
...
- `apps/web`: app shell, war-room map renderer, order sandbox running the engine locally.
