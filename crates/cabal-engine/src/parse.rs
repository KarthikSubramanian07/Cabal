...
fn tokenize(map: &Map, text: &str) -> Result<Vec<Tok>, ParseError> {
    ...
    for raw in cleaned.split_whitespace() {
        ...
        if let Some(p) = raw.strip_suffix(':') {
            let power = map
                .find_power(p)
                .ok_or_else(|| ParseError::UnknownPower(p.to_string()))?;
...
impl Parsed {
    ...
    pub fn to_movement(&self, default_power: Option<Power>) -> Result<Order, ParseError> {
        ...
        let command = match self.command {
            ...
            ParsedCommand::Move { dest, via_convoy } => Command::Move {
                dest: dest.province,
                coast: dest.coast,
                via_convoy,
            },
            ...
            ParsedCommand::Convoy { army, dest } => Command::Convoy {
                army: army.province,
                dest: dest.province,
            },
    ...
    pub fn to_retreat(&self, default_power: Option<Power>) -> Result<RetreatOrder, ParseError> {
        ...
        let region = self.region.ok_or(ParseError::Incomplete)?;
        let command = match self.command {
            ParsedCommand::Move { dest, .. } => RetreatCommand::Retreat {
                dest: dest.province,
                coast: dest.coast,
            },
    ...
    pub fn to_adjustment(&self, default_power: Option<Power>) -> Result<AdjustOrder, ParseError> {
        ...
        let command = match self.command {
            ...
            ParsedCommand::Build => AdjustCommand::Build {
                kind: self.kind,
                region: self.region.ok_or(ParseError::Incomplete)?,
            },
            ParsedCommand::Disband => AdjustCommand::Disband {
                unit: self.region.ok_or(ParseError::Incomplete)?.province,
            },
...
mod tests {
    ...
    fn errors() {
        ...
        assert!(matches!(
            parse(m(), "A XYZ - BUR"),
            Err(ParseError::UnknownProvince(_))
        ));
        assert!(matches!(parse(m(), "A PAR -"), Err(ParseError::Incomplete)));
        assert!(matches!(
            parse(m(), "A PAR - BUR - PIC"),
            Err(ParseError::Unexpected(_))
        ));
        let p = parse(m(), "A PAR S A BUR - PIC").unwrap();
        assert!(matches!(
            p.to_retreat(m().find_power("FRA")),
            Err(ParseError::NotInPhase("support"))
        ));
