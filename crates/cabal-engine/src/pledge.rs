...
pub fn canonical(orders: &[String]) -> String {
    let mut v: Vec<String> = orders
        .iter()
        .map(|o| o.split_whitespace().collect::<Vec<_>>().join(" ").to_ascii_uppercase())
        .filter(|o| !o.is_empty())
        .collect();
    v.sort();
    v.dedup();
...
pub fn receipt(pledged: &[String], submitted: &[String]) -> Receipt {
    ...
    if want.lines().all(|l| have.contains(l)) { Receipt::Kept } else { Receipt::Broken }
}
...
mod tests {
    ...
    fn canonical_is_order_and_case_insensitive() {
        assert_eq!(canonical(&s(&["a par - bur", "F  BRE  H"])), canonical(&s(&["F BRE H", "A PAR - BUR"])));
    }
    ...
    fn receipts() {
        ...
        assert_eq!(receipt(&pledged, &s(&["F KIE - DEN", "A MUN S F KIE - DEN"])), Receipt::Kept);
        assert_eq!(receipt(&pledged, &s(&["F KIE - DEN", "A MUN - BUR"])), Receipt::Broken);
